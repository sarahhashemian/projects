...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                ...
                Place curr = pl(i, j);
                _solnNumToPlace[seq] = curr;
                int wh =  _width * _height;
                int arD = arrowDirection(i, j);
                if (_solution[i][j] == 1 || _solution[i][j] == wh) {
                    _board[i][j] = new Sq(i, j, _solution[i][j], true, arD, -1);
                } else {
                    _board[i][j] = new Sq(i, j, 0, false, arD, -1);
                }
            }
        }
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                Sq sample = _board[i][j];
                sample._successors = allSuccessors(i, j, _board[i][j]._dir);
                sample._predecessors = _board[i][j].predecessors();
    ...
    Model(Model model) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                Sq copyS = model._board[i][j];
                int seqNum = copyS.sequenceNum();
                boolean f = copyS._hasFixedNum;
                int d = copyS._dir;
                _board[i][j] = new Sq(i, j, seqNum, f, d, copyS.group());
                _allSquares.add(_board[i][j]);
            }
        }
        for (int i = 0; i < _allSquares.size(); i++) {
            Sq newSq = model._allSquares.get(i);
            Sq sP = _allSquares.get(i);
            if (newSq._successor != null) {
                sP._successor = _board[newSq._successor.x][newSq._successor.y];
            } else {
                sP._predecessor = null;
            }
            if (newSq._predecessor != null) {
                int xpred = newSq._predecessor.x;
                int ypred = newSq._predecessor.y;
                sP._predecessor = _board[xpred][ypred];
            } else {
                sP._predecessor = null;
            }
            Sq headSQ = newSq._head;
            if (headSQ != null) {
                sP._head = _board[headSQ.x][headSQ.y];
            } else {
                sP._head = null;
            }
    ...
    boolean autoconnect() {
        for (int i = 0; i < this._width; i++) {
            for (int j = 0; j < _height; j++) {
                if (_board[i][j]._sequenceNum != 0) {
                    for (int x = 0; x < this._width; x++) {
                        for (int y = 0; y < _height; y++) {
                            int bSucSN = _board[x][y].sequenceNum() - 1;
                            boolean b = _board[i][j].connectable(_board[x][y]);
                            if (_board[i][j].sequenceNum() == bSucSN && b) {
    ...
    void solve() {
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int seqn = _solution[i][j];
                this._board[i][j]._sequenceNum = seqn;
                if (this._board[i][j]._sequenceNum != _width * _height) {
                    this._board[i][j]._successor = solnNumToSq(seqn + 1);
                }
                if (this._board[i][j]._sequenceNum != 1) {
                    this._board[i][j]._predecessor = solnNumToSq(seqn - 1);
                }
            }
        }
        _unconnected = 0;
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (s1._sequenceNum == 1 || this._sequenceNum == _width * _height) {
                return false;
            }
        ...
        boolean connect(Sq s1) {
            ...
            Sq suc = this.successor();
            while (suc != null) {
                suc._head = this._head;
                suc = suc.successor();
            }
            return true;
        }
         ...
         *  @param num is s1 seqnumber */
        void addNum(Sq s1, int num) {
            s1._sequenceNum = num + 1;
            if (s1._successor != null) {
                addNum(s1._successor, num + 1);
            }

        }
        /** @param s1 is a square.
         *@param num is s1 seqnumber */
        void subtractNum(Sq s1, int num) {
            s1._sequenceNum = num - 1;
            if (s1._predecessor != null) {
                subtractNum(s1._predecessor, num - 1);
            }
        }

        /** Disconnect this square from its current successor, if any. */
        void disconnect() {
            Sq nt = _successor; if (nt == null) {
                return;
            }
            _unconnected += 1; nt._predecessor = _successor = null;
            if (_sequenceNum == 0) {
                if (this._predecessor == null && nt._successor == null) {
                    releaseGroup(this._group); releaseGroup(nt._group);
                    this._group = -1; nt._group = -1;
                } else if (this._predecessor == null) {
                    this._group = -1;
                } else if (nt._successor == null) {
                    nt._group = -1;
                } else {
                    nt._group = newGroup();
                }
                ...
                if (!this._hasFixedNum) {
                    Sq p = this.predecessor(); boolean h = false;
                    if (p != null) {
                        h = p._hasFixedNum;
                    }
                    while (p != null) {
                        h = p._hasFixedNum; p = p._predecessor;
                    } if (!h) {
                        this._sequenceNum = 0; p = this._predecessor;
                        if (p != null) {
                            this._head._group = newGroup();
                            _usedGroups.add(this._head._group);
                            while (p != null) {
                                p._sequenceNum = 0; p = p._predecessor;
                            }
                        } else {
                            releaseGroup(this._head._group); this._group = -1;
                            this._head = this;
                        }
                    }
                }
                if (!nt._hasFixedNum) {
                    boolean n = false; Sq s = nt.successor(); if (s != null) {
                        n = s._hasFixedNum;
                    }
                    while (s != null) {
                        n |= s._hasFixedNum; s = s._successor;
                    } if (!n) {
                        nt._sequenceNum = 0; s = nt._successor; if (s != null) {
                            nt._group = newGroup(); _usedGroups.add(nt._group);
                            while (s != null) {
                                s._sequenceNum = 0; s = s._successor;
                            }
                        } else {
                            releaseGroup(nt._head._group); nt._group = -1;
                        }
                    }
                }
            }
            nt._head = nt; Sq ntSuc = nt.successor(); while (ntSuc != null) {
                ntSuc._head = nt; ntSuc = ntSuc.successor();
            }
        }
        @Override
        public boolean equals(Object obj) {
